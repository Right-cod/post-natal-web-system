...
<section class="justify-content-center text-center" id="status">
    <div class="bodyt">   
<p style="color:white;">Outspan apointment status.</p>
<?php
$email=$row['email'];
$sq=mysqli_query($con,"SELECT * FROM appointment WHERE email='$email' ORDER BY application_day ASC  ");
while ($wer=mysqli_fetch_array($sq)){
  $received=$wer['reply'];
  $on=$wer['posted'];
  $ty=$wer['time_posted'];
  $replied=$wer['reply_day'];
  $sent=$wer['application_day'];
?>
<!--send-->
<div class="row float-left m-2 w-100">
<div class="w-50 mr-auto bg-dark sendrow">
<p style="width:auto;padding:20px;"><?php echo $wer['message'];?></p>
...
<p style="width:auto;padding:20px;"><?php echo "Dear $name, $received $on $ty";?></p>
