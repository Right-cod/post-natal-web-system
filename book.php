...
<div class="wait overlay">
        <div class="loader"></div>
            ...
            <h2 class="text-center color-blue">Book Appointment Now.</h2>
            <h6 style="color:white;background:green; padding:20px;" ><i class="far fa-bell"></i> A booking fee of =/KSH100 will be required upon booking. Aprompt will be sent to your phone for authentication. </h6>
  <form onsubmit="return false" id="book" class="form justify-content-center text-center">
  <div class="col-md-12 text-center " style="width:100%;" id="book_msg">
                    <!--Alert from signup form-->
      </div>
      <input type="text" name="id" hidden id="id" value="<?php echo $row['user_id']; ?>">
      <label for="" class="text-left  w-50 " >Email:</label>
      <input type="text" name="email" placeholder="Email eg abxs@...." value="<?php echo $row['email']; ?>" class="flname mt-2 mb-2 w-50">
      <label for="" class="text-left  w-50 " >Phone:</label>
      <input type="text" name="mobile" placeholder="Phone eg. 07...." value="<?php echo $row['mobile']; ?>" class="flname mt-2 mb-2 w-50">
      <label for="" class="text-left  w-50 " >Department:</label>
      <select  id="" name="department" class="flname mt-2 mb-2 w-50">
      <option value="Select Department">Select Department</option>
      <option value="Consultation">Consultation</option>
      <option value="Immunization">Immunization</option>
      <option value="Psycology">Psycology</option>
      </select>
      <label for="" class="text-left  w-50 " >Date of delivery:</label>
      <input type="date" name="date" placeholder="" value="" class="flname mt-2 mb-2 w-50">
      <label for="" class="text-left  w-50 " >Message:</label>
      <textarea name="message" id="" cols="30" rows="5" class="flname mt-2 mb-2 w-50" placeholder="Type your application message here....."></textarea>
      <input type="submit" value="Book Appointment" class="btn btn-success m-3 w-50">
  </form>
</div>
</section>
<!--booking end-->
...
<script>
   ...
   $(".overlay").show();
   $("#book_msg").html('<div style="color:chocolate;"><h3>Please wait as we process your request.........</h3></div>');
   $.ajax({
      url : "book_now.php",
      method : "POST",
      data : $("#book").serialize(),
      success : function(data){
         ...
         if (data == "register_success") {
          $("#book_msg").html('<div style="color:green;"><h3>Book success. An email has been sent to you to confirm your booking.</h3></div>');
           setTimeout(function(){
